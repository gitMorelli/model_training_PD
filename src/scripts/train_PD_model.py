...
from src.utils.training_utils import LitModel, BestMetricTracker
...
exp_params = {
    ...
    'grid_dict_path': "/mnt/beegfs02/scratch/a_morelli/datasets/rr_data_h5.pkl",
    ...
    'majority_class_id': 0, #0 -> 5467, #1  ->  1145
    'threshold_num': 1,
...
#clip-vit-large-patch14, clip-vit-large-patch14-inter
    ...
    'custom_transform': None, #if not None overrides the transform defined for the model with ta custom one
    ...
    'apply_augmentation': None, #None, 'random_crop_half' 
    'invert_color':True,
...
def main():
    ...
    train_loader,val_loader,train_dataset,val_dataset= prepare_loaders(worker,prefetch_factor,exp_params,exclusion_set,val_exclusion_set, grid_dict)
    
    if DEBUG_IMGS:
        debug(train_loader,val_loader,exp_params)
    

    example_input_array = torch.randn(1, exp_params['num_tiles'], 3, 224, 224)
    lit_model = LitModel(write_log,model=model, num_0=num_0, num_1=num_1, num_classes=exp_params['num_classes'],lr_backbone=exp_params['lr_backbone'], 
...
def prepare_loaders(worker,prefetch_factor,exp_params,exclusion_set,val_exclusion_set, grid_dict):
    ...
    train_dataset = prepare_PD_dataset(SHARD_PATTERN_train, split_workers=True, batch_size=exp_params['batch_size'], transform=None, exclusion_set=exclusion_set, modality=exp_params['data_modality'],
    ...
    val_dataset = prepare_PD_dataset(SHARD_PATTERN_val, split_workers=True, batch_size=exp_params['batch_size'], transform=None, exclusion_set=val_exclusion_set, modality=exp_params['data_modality'],
                       huggingface_transform=exp_params['huggingface_transform'],augmentation_transform=augmentation_transform, 
                       invert_color=exp_params['invert_color'],n_views=exp_params['num_tiles'], grid_dict = grid_dict)
...
def prepare_exclusion_sets(exp_params,verbose=True):
    ...
    assert 1==0, "Debugging: Stopping after CSV data inspection. Check the printed information above."
...
def get_augmentation_transform(exp_params):
    if exp_params['apply_augmentation'] is None:
        return None
    elif exp_params['apply_augmentation'] == 'random_crop_half':
        return T.Compose([
            T.RandomCrop(
